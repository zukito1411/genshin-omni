...
export interface GenshinCharacter { id:string; name:string; rarity?:number; element?:ElementName; weapon?:string; region?:string; birthday?:string; title?:string; description?:string; quote?:string; images:{card:string;icon:string;portrait:string}; raw:Record<string,unknown>; }
...
export interface CharacterGuide { characterId:string; role:string[]; summary:string; statPriority:string[]; talentPriority:string[]; weapons:Array<{name:string;tier:'Signature'|'5★'|'4★'|'F2P'|'Recommended';note:string;source?:string}>; artifacts:Array<{set:string;pieces:string;note:string;source?:string}>; mainStats:{sands:string;goblet:string;circlet:string}; teams:Array<{name:string;members:string[];note:string}>; caveats?:string[]; sourceLinks?:Array<{label:string;url:string}>; }
