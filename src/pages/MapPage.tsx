...
export function MapPage(){
 ...
 useEffect(()=>{
   ...
   fetchLiveMapMarkers(controller.signal).then(setLiveMarkers).catch((error)=>{
 ...
 const matchedMarkers=useMemo(()=>{const query=mapSearch.trim().toLowerCase();return liveMarkers.filter((marker)=>!query||`${marker.name} ${marker.type} ${marker.mapId}`.toLowerCase().includes(query))},[liveMarkers,mapSearch]);
 const visibleMarkers=matchedMarkers.slice(0,600);
 return <div><SectionTitle eyebrow="EXPLORATION" title="Interactive Map & Personal Progress" description="Find resources, bosses, specialties and exploration markers, then keep your own farming notes in this browser." />
   <section className="map-embed panel"><div className="map-embed-head"><div><div className="eyebrow">WORLD MAP</div><h3>Community marker map</h3></div><a className="source-button" href={MAPGENIE_URL} target="_blank" rel="noreferrer">Open MapGenie <ExternalLink size={13}/></a></div>
     <form className="map-search" onSubmit={submitMapSearch}><Search size={15}/><input value={mapSearch} onChange={(e)=>setMapSearch(e.target.value)} placeholder="Search chests, shrines, time trials, crimson agate..." /><button className="button primary" type="submit">Search</button><button type="button" className="button secondary" onClick={clearMapSearch} disabled={!mapSearch}>Clear</button></form>
     <div className="live-map-wrap">
      {mapLoading&&<div className="map-loading-state"><div className="map-loading-spinner"/><strong>Loading community markers…</strong><span>Downloading the current public marker data for this local map.</span></div>}
      ...
      {!mapLoading&&!mapError&&<div className="live-map-canvas">{['Mondstadt','Liyue','Inazuma','Sumeru','Fontaine','Natlan'].map((label,index)=><span key={label} className={`world-label world-${index+1}`}>{label}</span>)}{visibleMarkers.map((marker)=><button type="button" className={`live-map-marker ${selectedMarker?.id===marker.id?'selected':''}`} key={marker.id} style={{left:`${marker.x*100}%`,top:`${marker.y*100}%`}} title={marker.name} onClick={()=>setSelectedMarker(marker)}><MapPinned size={11}/></button>)}{selectedMarker&&<div className="live-map-detail"><strong>{selectedMarker.name}</strong><span>Map {selectedMarker.mapId} · level {selectedMarker.level}</span><button type="button" onClick={()=>setSelectedMarker(null)}>Close</button></div>}</div>}
     </div>
     <div className="map-help"><span>{mapLoading?'Loading markers…':`${matchedMarkers.length.toLocaleString()} matching markers · showing up to ${visibleMarkers.length}`}</span><span>Click a pin for details. Marker data is loaded directly, not embedded.</span></div>
   </section>
